    ...
    for idx in range(0,len(order)):
        ...
        if idx in to_replace: # smiles to replace (by better molecules)
            ...
            if np.random.random() < 0.5: 
                ...
                f = open('./geneology/gen_11.txt', 'a+')
                f.writelines(['{} mut: {}\n'.format(smiles_new, smiles_[random_index])])
                f.close()
                
                # TODO: Check that the mutaion has no violations (Robert's Filter)
            ...
            # CROSSOVER WITH A GOOD MOL: 
                ...
                # smiles_mutated.append(crossover_smi)
...
with open('gen_10_all.smi', 'r') as f: # TODO: FOR GENERATION 2! 
    content = f.readlines()
...
for x in singlet_trip: 
    if x>= 0.6: prop_1.append(-10**6)
    else: prop_1.append(0.6-x) 
...
to_replace = to_replace[0:5000] # TODO


# order, to_replace, to_keep = order[0:5], to_replace[0:5], to_keep[0:5] # TODO: Remove after testing! 
print('Keeping: {} Replacing: {}'.format(len(to_keep), len(to_replace)))
# raise Exception('Testing success rate!')



smiles_next_gen = obtain_next_gen_molecules(order, to_replace, to_keep, smiles_)

# raise Exception('Testing ordereed smiles;')
...
f = open('./gen_11_unique.smi', 'a+')
f.writelines(['{} {}\n'.format(item, i+10000) for i,item in enumerate(canon_smi_ls)])
f.close()



# WRITING ALL THE SMILES! 
f = open('./gen_11_all.smi', 'a+')

...
def substructure_preserver(mol):
    ...
    """        
    if mol.HasSubstructMatch(Chem.MolFromSmarts('*1***2****-2**1')) == True: 
...
def eval_smi(test_mol): 
    model_testing = torch.load('./saved_models_st/34/model_checkpoint.pt'.format(3))
    model_testing.eval()
...
def crossover(smi_1, smi_2):     
    ...
    for item in best_smi: 
        try: 
            if substructure_preserver(Chem.MolFromSmiles(item))==True and passes_filter(item) == True:
    ...
    if filter_pass == []:
        ...
        best_smi, best_scores = get_median_mols(smi_1, smi_2, 5, 5, collect_bidirectional, 5000, apply_filter=apply_filter)
        filter_pass = []
        for item in best_smi: 
            try: 
                if substructure_preserver(Chem.MolFromSmiles(item))==True and passes_filter(item) == True:

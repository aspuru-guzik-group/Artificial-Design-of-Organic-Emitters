...
for key in smiles_: 
    ...
    excit_energy.append(smi_to_prop[key][2]) # TODO!!
...
prop_3 = []
for x in excit_energy: 
    if x< 0.0: prop_3.append(-10**6)
    else: prop_3.append(-np.abs(x-3.2)) 
    
fitness = np.array(prop_1) + np.array(prop_2) + np.array(prop_3)
